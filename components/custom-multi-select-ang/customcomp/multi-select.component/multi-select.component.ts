...
export class MultiSelectComponent implements OnInit {
    ...
    public multiSelectData: string[] = countries;
    public multiSelectData2: string[];
    ...
    ngOnInit() {
        ...
        // If we could call into the base view we could get the data this way
        // but that would be an undesirable coupling
        // strangely we can make that call directly from the template (components/custom-multi-select-ang/angular/template.html.ejs)
        /*theview.getDataChanges(this.dataSourceName).subscribe(
            (data) => { this.multiSelectData2.push(data); },
            (error) => { alert (`Error: ${error}`); },
            () => {
                console.log(`completed subscription ${this.multiSelectData2.length}`);
            }
        );
        */
    }
